...
### 1.3 責任分界（TimeRex・GAS・L-step）

- **予約の作成・キャンセル**は **TimeRex の機能**。キャンセル操作は TimeRex のキャンセルURLや管理画面で実行される。
- **本システム（GAS）**は、TimeRex の Webhook（`event_confirmed` / `event_cancelled`）を**受信**し、スプレッドシートの更新と L-step 連携を行う。キャンセルを「実行」するのではなく「通知に反応」するだけである。
- **キャンセル時にリマインドを解除する**には、GAS が L-step に `meeting_date: null` 等を送り、L-step のリマインド条件（`meeting_date` に値があること）から外す。

詳細は [TimeRex・GAS・L-step 責任分界とリマインド連携設計](design/TIMEREX_GAS_LSTEP_DESIGN.md) を参照。
...
1. Lステップから応募者に面談予約案内を配信（ステップ配信機能）
   ...
   - ボタンタップでLステップが本システム（Webhook転送URL）へPOST ⇒ UIDを受信し `session_id` を発行（CacheService + `uidlog` シート）
...
**手動に関する運用の整理（フロー・Lステップ連携・KPI・リマインダー方針）は [運用: 手動予約・手動操作の扱い](operations/MANUAL_BOOKING_AND_OPERATIONS.md) を参照。**
...
### 3.3 uidlog（UIDセッション管理）

LステップのWebhook転送で取得したUIDとセッションIDの対応を管理するシート。

| 列 | カラム名 | 説明 | 例 |
|---|---|---|---|
| A | 日時 | 記録日時 | `2026-02-11 11:53:35` |
| B | `uid` | LINEユーザーID | `U6e967fdb7...` |
| C | `sessionid` | セッションID（UUID） | `9d94a869-4206-495f-bafb-9f0dcd75cba2` |
| D | イベント種別 | 種別（postback 等） | `postback` |

**用途:**
- `handleLStepWebhook` が postback/Webhook転送を受信したタイミングで `session_id` を発行し、CacheServiceと併せて uidlog に保存
- `handleBookingPage` が `session_id` から UID を逆引き（CacheService 優先、uidlog フォールバック）し、TimeRexウィジェットの `url_params.line_uid` にセット
- `getMostRecentSession(120)` で直近2分以内の1件を取得（uidlog に interviewer_id は保存しないため、セッション検索は常に直近1件）。詳細は [LSTEP_WEBHOOK_SPEC.md](lstep/LSTEP_WEBHOOK_SPEC.md) を参照
...
function doGet(e) {
  ...
  if (sessionId) {
    // uidlogシートからUIDを取得（カラム: 日時, uid, sessionid, イベント種別, friendid）
...
```
1. ユーザーが postback ボタンをタップ → Webhook転送（POST）でUID取得 → `session_id` 生成（CacheService + uidlog）
   ↓
2. Lステップが応募者に予約URLをメッセージ送信 → ユーザーがURLを開く（推奨: `?from=line`）
   ↓
3. GAS doGet() が直近2分以内のセッション1件を getMostRecentSession(120) で取得し、リダイレクトせず handleBookingPage(session_id) のHTMLを直接返す
   ↓
4. handleBookingPage が session_id から CacheService / uidlog を参照して UID を取得
   ↓
...
| UIDセッション保存 | CacheService + `uidlog` シート | 有効期限10分（Cache）、直近2分以内で getMostRecentSession により検索。詳細は [LSTEP_WEBHOOK_SPEC.md](lstep/LSTEP_WEBHOOK_SPEC.md) を参照 |
...
2. **postbackボタン**  
   ...
   - タップ後にLステップ側のシナリオで予約URLを送信。**推奨**: `...?from=line`（必要なら `&interviewer_id=tanaka`）。`?action=lstep_webhook` だと一部環境でPC表示になる場合あり。
...
（詳細は [LSTEP_WEBHOOK_SPEC.md](lstep/LSTEP_WEBHOOK_SPEC.md) を参照。以下は要約。）

1. **postbackタップ**  
   - Lステップ → GASへWebhook転送（POST）。`handleLStepWebhook` が UID を抽出。  
   - `session_id` を発行し、CacheService（600秒）と **uidlog** シート（日時, uid, sessionid, イベント種別, friendid）へ保存。

2. **予約URLの送付**  
   - Lステップのシナリオが `...?from=line`（必要なら `&interviewer_id=xxx`）を応募者へ送信。

3. **応募者がURLを開く（GET）**  
   - `doGet` が `from=line` または `action=lstep_webhook` を検知し、**直近2分以内のセッション1件**を `getMostRecentSession(120)` で取得（uidlog に interviewer_id は保存しないため常に直近1件）。  
   - 成功時は**リダイレクトせず**、同一リクエスト内で `handleBookingPage({ parameter: { session_id } })` を呼び、**予約画面のHTMLを直接返す**（中間リダイレクトだとPC表示になるため）。見つからない場合は「セッションが見つかりません」のエラーページを返す。

4. **予約画面表示**  
   - `handleBookingPage` が `session_id` から UID を復元（CacheService 優先、uidlog フォールバック）。  
   - テンプレートの userData.uid を TimeRex ウィジェットの `url_params.line_uid` に設定。
...
- 予約URLクリック後に「セッションが見つからない」場合はタップから2分以内か、`uidlog` シートにレコードがあるか（および GET/POST で同じ SPREADSHEET_ID か）を確認。  
...
- セッションIDとUIDの対応関係をuidlogシートに保存
  - **保存項目**: `日時`, `uid`, `sessionid`, `イベント種別`, `friendid`
  - **メリット**: 
...
│ 保存先: uidlogシート（日時, uid, sessionid, イベント種別, friendid）         │
└─────────────────────────────────────────────────────────────────┘
                            ↓
┌─────────────────────────────────────────────────────────────────┐
│ 【ステップ2】予約システムアクセス時                               │
...
**uidlogシート（`uidlog`）:**
- カラム: `日時`, `uid`, `sessionid`, `イベント種別`, `friendid`
